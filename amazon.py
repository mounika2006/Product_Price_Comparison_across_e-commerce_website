...
# def get_amazon_price(product_name: str, serpapi_key: str = "5f583e385ed06e0912c2d828d5c600adec067cf85328632352b2cd1a5bc36b3b") -> Dict[str, any]:
#     """
...
#                 "asin": result["asin"],
#                 "rating": f"{result['rating']}" if result["rating"] else "N/A",
...
#     API_KEY = "5f583e385ed06e0912c2d828d5c600adec067cf85328632352b2cd1a5bc36b3b"
#     result = get_amazon_price("mouse")
...
#         self.product_synonyms = {
#             'refrigerator': ['mini refrigerator','mini fridge','fridge', 'single door fridge'],
#             'electric fan': ['fan', 'ceiling fan', 'pedestal fan', 'wall fan', 'table fan'],
#             'iphone': ['iphone', 'apple iphone'],
#             'samsung galaxy': ['samsung galaxy', 'galaxy s', 'samsung s'],
#             'backpack': ['backpack', 'rucksack', 'school bag', 'laptop bag','bag'],
...
#             'air conditioner': ['ac', 'air conditioner', 'split ac', 'window ac'],
#             'jean': ['women jeans','jeans','baggy jeans','straight jeans', 'denim jeans'],
#             'running shoes': ['shoes','shoe','sneakers', 'running shoes', 'sports shoes', 'trainers'],
#             'mouse': ['mouse', 'wireless mouse', 'usb mouse', 'gaming mouse']
#         }
...
#                 break
#         variations.append(product_name.replace(' ', '').replace('_', ''))
#         return list(dict.fromkeys(variations))[:6]

#     def get_price_ranges(self, product_name: str) -> tuple:
#         product_words = product_name.lower().replace('_', ' ').split()
#         if any(w in product_words for w in ['fridge', 'refrigerator']): return 7000, 100000
#         elif any(w in product_words for w in ['fan']): return 800, 8000
#         elif any(w in product_words for w in ['jean', 'jeans']): return 200, 5000
#         elif any(w in product_words for w in ['shoes', 'sneakers']): return 300, 10000
#         elif 'mouse' in product_words: return 150, 5000
#         else: return 100, 100000

#     def is_main_product(self, title: str, product_name: str) -> bool:
#         title_lower = title.lower()
#         # Block accessories and irrelevant items
#         blocks = ['case', 'cover', 'charger', 'cable', 'screen protector', 'glass', 'belt', 'wallet', 'pad', 'sticker']
#         if any(block in title_lower for block in blocks):
#             return False
#         return True 
...
#                     if min_price <= price_num <= max_price:
#                         # Extract Details
#                         curr_rating = result.get("rating", 0)
#                         curr_reviews = result.get("reviews", 0)
#                         delivery = result.get("delivery", "Check delivery date on site")
                        
#                         redirect_link = self.create_redirect_link(result.get("link"), result.get("asin"))
...
#         # --- 🔥 STRICT FILTER LOGIC 🔥 ---
#         # We want the cheapest item that ALSO has a rating.
        
#         selected_product = None
        
#         # Pass 1: Find Cheapest item with Rating > 3.0 (Avoids bad products)
#         for prod in all_products:
#             if prod['rating'] >= 3.0:
#                 selected_product = prod
#                 break
        
#         # Pass 2: If no good ratings, find Cheapest item with ANY rating > 0
...
#         # Pass 3: Fallback to absolute cheapest if literally nothing has a rating
#         if not selected_product:
#             selected_product = all_products[0]

#         result_data = selected_product['result']
#         is_prime = "Prime" in result_data.get('title', '') or result_data.get('is_prime')
        
#         return {
#             "lowest_price": f"₹{selected_product['price']:,.0f}",
#             "price_value": selected_product['price'],
#             "title": result_data.get("title", ""),
#             "link": selected_product['redirect_link'],
#             "asin": result_data.get("asin"),
...
#             "is_prime": is_prime,
#             "total_checked": len(all_products),
#             "best_search_term": selected_product['search_term']
#         }
...
#                 "link": result["link"],
#                 "asin": result["asin"],
#                 # IMPORTANT: Return keys exactly as HTML expects them
#                 "rating": result['rating'] if result['rating'] > 0 else None,
#                 "reviews": f"{result['reviews']:,}" if result['reviews'] else "0",
#                 "delivery": result.get("delivery"), # This now contains actual text like "Get it by Tomorrow"
#                 "image": result["thumbnail"],
#                 "total_checked": result["total_checked"]
#             }
#         else:
#             return {
#                 "success": False, "price": "No products found", "title": product_name,
#                 "rating": None, "reviews": "0", "delivery": "N/A", "image": ""
#             }
#     except Exception as e:
#         logger.error(f"Amazon Error: {e}")
#         return {
#             "success": False, "price": "Error", "title": str(e),
#             "rating": None, "reviews": "0", "delivery": "N/A", "image": ""
#         }

# if __name__ == "__main__":
#     res = get_amazon_price("mouse")
...
def get_amazon_price(product_name: str, serpapi_key: str = "0c64bffa3140c19ac328ccb2f02f10c6b7ec8fbfa9e6bd58fd6899a0a6279a2e") -> Dict[str, any]:
    """
    MAIN FUNCTION for app.py - Returns ALL product details as dict
    """
    try:
